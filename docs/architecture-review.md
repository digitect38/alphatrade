...
| # | 과제 | 분류 | 상태 |
|---|---|---|---|
| 1 | 리스크 정책 강제 적용 | A-1 | ✅ TradingGuard (세션/stale/섹터/참여율) |
| 2 | 킬 스위치 (자동+수동) | A-5 | ✅ 일간 손실 자동 + API 3개 + 브로커 회로차단 |
| 3 | 주문 FSM + 멱등키 + 재시작 복구 | A-2,3 | ✅ 9단계 상태 + SHA-256 멱등키 |
| 4 | 버전 고정 | A-7 | ✅ Docker 이미지 pinned |
| 5 | 불량 데이터 차단 | A-4 | ✅ 30초 stale → 진입 차단 |
| 6 | 불변 감사 로그 | A-6 | ✅ audit_log (append-only) |
| 7 | 백업/DR 스크립트 | B-1 | ✅ 전체/증분/복원 스크립트 |
| 8 | KIS WebSocket 실시간 | 기능 | ✅ wss → Redis → /ws → Dashboard |
| 9 | WebSocket 대시보드 | 기능 | ✅ useWebSocket + Market.tsx LIVE |
| 10 | n8n 워크플로우 (10/10) | 자동화 | ✅ WF-01~WF-10 전체 |
| - | VPS 분리 | A-8 | ⚠️ 운영 작업 (코드 외) |
