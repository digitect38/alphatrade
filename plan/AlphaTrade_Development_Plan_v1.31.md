...
- 문서 버전: v1.31 (Commercial Controls Normalized)
- 작성일: 2026-03-30 (v1.31 업데이트)
...
| 모듈간 실시간 (현재) | Redis Pub/Sub | 시세/시그널 이벤트 발행 위주. 비내구성, 재시작 시 replay 불가 |
| 모듈간 실시간 (실전 필수) | Redis + DB/WAL 기반 내구성 이벤트 저장 | 주문/포지션/리스크 이벤트를 append-only 저장하고 재처리 가능하게 구성 |
| React 대시보드 ↔ 백엔드 (현재) | REST Polling | 현재 가격/상태 조회는 주기적 polling 기반 |
| React 대시보드 ↔ 백엔드 (목표) | WebSocket + REST API | 실시간 차트 업데이트는 WebSocket, 조회/제어는 REST 사용 |
...
| 컨테이너 | 이미지 | 포트 | 의존성 |
|---|---|---|---|
| timescaledb | timescale/timescaledb:2.16.1-pg16 | 5432 | - |
| redis | redis:7-alpine | 6379 | - |
| n8n | n8nio/n8n:1.82.1 | 5678 | timescaledb, redis |
| core-engine | custom (Python 3.12 + FastAPI) | 8000 | timescaledb, redis |
| dashboard | custom (React + Nginx) | 3000 | core-engine |
| prometheus | prom/prometheus:v2.52.0 | 9090 | core-engine, n8n |
| grafana | grafana/grafana:11.0.0 | 3001 | prometheus, timescaledb |
...
- 실전 배포 기준 이미지는 `latest` 사용 금지, 스테이징 검증 후 고정 태그로만 승격한다.
...
맥북을 서버로 사용할 때 가장 큰 위험은 슬립, 네트워크 끊김, macOS 업데이트 등으로 장중 시스템이 중단되는 것이다. v1.31 기준으로 실전 파일럿부터는 주문 실행 엔진을 서버에서 우선 실행하며, 맥북은 개발/연구/모니터링 허브로만 사용한다.
...
- **통신**: 연구/분석 노드(MacBook 또는 별도 서버)는 VPS의 전략/주문 API로 신호를 전달한다. 실전 주문 이벤트는 Redis와 DB append-only 로그에 이중 기록한다.
- **VPS 독립 동작**: 분석 노드가 끊겨도 VPS는 기존 포지션의 손절/익절 감시와 브로커 상태 조정을 독립적으로 수행한다.
- **운영 원칙**: 개발/페이퍼 단계에서만 MacBook 단독 운영을 허용하고, 소액 실전 파일럿 시작 시점(Phase A)부터 VPS 분리를 강제한다.
- **비상 시나리오**: 분석 노드 장애 시 VPS가 자동으로 전체 포지션 보호 모드(신규 진입 차단, 기존 손절만 관리) 진입한다.
...
| 클라우드 VPS (실전 단계) | 월 ~₩30,000~50,000 | Phase A부터 주문 실행 분리 시 |
...
### 16.5 실전 파일럿 통제 정의 (v1.31 기준)

아래 기준은 `소액 실전 파일럿`의 기본값이다. 전략별 특성에 따라 더 보수적으로 조정할 수 있으나, 이 기준보다 느슨한 값으로는 실전 승인하지 않는다.

#### 16.5.1 리스크 정책 기본값

| 항목 | 기본값 | 강제 방식 |
|---|---|---|
| 일간 총손실 한도 | 계좌 기준 `-2.0%` 도달 시 신규 진입 차단 | 킬 스위치 자동 발동 |
| 전략별 일간 손실 한도 | 전략 자본 기준 `-1.0%` | 전략 단위 자동 중지 |
| 종목당 최대 투입 | 총 실전 자본의 `10%` | 주문 생성 전 차단 |
| 섹터 집중 한도 | 총 실전 자본의 `25%` | 포지션 집계 후 차단 |
| 단일 주문 최대 참여율 | 최근 20일 평균 거래대금 대비 `1%` 이내 | 유동성 필터 |
| 손절 기준 | 엔트리 기준 `-1.5%` 또는 전략 ATR 기반 더 보수적 값 | 주문/모니터 엔진 강제 |
| 신규 진입 허용 시간 | 정규장 기준 `09:05` 이후 ~ `15:00` 이전 (KOSPI/KOSDAQ 정규장 09:00-15:20, 장 개시 후 5분·장 마감 전 20분 적용). 동시호가(08:30-09:00, 15:20-15:30), 장전 시간외(08:00-08:30), 장후 시간외(15:40-16:00) 구간에서는 신규 진입 차단. 상세: `docs/market-calendar.md` | 세션 가드 |
| 오래된 시세 차단 | 마지막 유효 체결가 기준 `30초` 초과 stale 시 신규 주문 차단 | 데이터 게이트 |
| 브로커 API 연속 실패 | `3회` 연속 실패 시 신규 주문 차단, 운영자 호출 | 자동 세이프가드 |

#### 16.5.2 주문 FSM 및 브로커 조정 규칙

| 항목 | 정의 |
|---|---|
| 주문 상태 | `created -> validated -> submitted -> acked -> partially_filled -> filled / cancelled / rejected / expired / unknown` |
| 멱등키 | 모든 주문 요청은 `strategy_id + trading_day + symbol + side + intent_seq` 조합으로 생성 |
| submit 타임아웃 | 브로커 ACK 미수신 `3초` 초과 시 `unknown` 상태로 전환 후 조회 API로 재확인 |
| 부분 체결 처리 | 부분 체결 시 잔량 기준으로 리스크 재계산 후 유지/취소 결정 |
| 브로커 조정 | 장 종료 후 포지션, 현금, 미체결 주문을 브로커 원장과 자동 대조 |
| 재시작 복구 | 프로세스 시작 시 `submitted/acked/partially_filled/unknown` 상태 주문을 우선 복구 |

#### 16.5.3 감사 로그 및 보관

| 항목 | 정의 |
|---|---|
| 저장 대상 | 시세 수신, 전략 판단, 리스크 판단, 주문 요청, 브로커 응답, 수동 개입 |
| 저장 방식 | DB append-only 테이블 + 일 단위 압축 아카이브 |
| 필수 필드 | `event_id`, `event_time`, `source`, `strategy_id`, `symbol`, `payload_hash`, `operator_id`, `correlation_id` |
| 보관 기간 | 운영 로그 `1년`, 주문/체결/정산 로그 `5년` 기본 보관 |
| 무결성 | 로그 레코드 수정 금지, 정정은 보정 이벤트로만 수행 |
...
| RPO | `5분` 이내 |
| RTO | `30분` 이내 |
| 백업 주기 | DB 증분 `5분`, 전체 스냅샷 `일 1회` |
| 복원 점검 | 월 `1회` 스테이징 복원 테스트 |
| 장애 모드 | 핵심 서비스 장애 시 신규 진입 차단, 기존 포지션 보호 모드 유지 |

#### 16.5.5 보안 운영 기준

| 항목 | 기준 |
|---|---|
| RBAC | `admin`, `operator`, `viewer`, `service` 4개 역할 분리 |
| 시크릿 로테이션 | 브로커/API 키 `90일`, 내부 서비스 키 `30일` 주기 |
| 배포 승인 | 실전 브랜치 배포는 1인 작성 + 1인 승인 원칙 |
| 취약점 점검 | 이미지/의존성 스캔을 주 1회, 실전 배포 전 필수 수행 |
| 웹훅 보호 | HMAC 서명 검증 + timestamp 허용오차 `60초` |
...
| 순위 | 과제 | 분류 | 영향도 |
|---|---|---|---|
| 1 | 리스크 정책 문서화 + 강제 적용 | 상용 A-1 | 🔴 안전 |
| 2 | 킬 스위치 (자동+수동) | 상용 A-5 | 🔴 안전 |
| 3 | 주문 FSM + 브로커 조정 | 상용 A-2,3 | 🔴 안전 |
| 4 | 버전 고정 + VPS 분리 | 상용 A-7,8 | 🔴 운영 |
| 5 | 불량 데이터 차단 | 상용 A-4 | 🟠 안전 |
| 6 | 불변 감사 로그 | 상용 A-6 | 🟠 추적 |
| 7 | 백업/복원/DR | 상용 B-1 | 🟠 운영 |
| 8 | 실시간 주가 수신 (KIS WebSocket) | 기능 | 핵심 기능 |
| 9 | WebSocket 대시보드 실시간 | 기능 | UX |
| 10 | n8n 워크플로우 확장 (9개) | 자동화 | 아키텍처 |
...
| v1.31 | 2026-03-30 | **상용 통제 정규화**: 서버 우선 실행 원칙을 본문 전체에 반영, Docker 이미지 고정 태그 명시, 현재/목표 실시간 아키텍처 분리, 실전 파일럿 통제 정의 추가 (리스크 한도, 주문 FSM, 브로커 조정, 감사 로그, 백업/RTO/RPO, RBAC/시크릿 로테이션), 우선순위 재정렬 |
